...
from datasets.datasets import get_FashionMNIST_dataloader
...
def solver(args, model, train_loader, test_loader):
	...
	for e in range(args.epoch):
		...
		model.eval()
		ACC, PURITY, NMI, ARI = evaluate(model, test_loader)  # evaluation on test_loader
		acc_list.append(ACC)
...
if __name__ == '__main__':
	...
	parser.add_argument('--clustering', type=str, default='kmeans', help='choose a clustering method (default: kmeans)' \
					   ' meanshift, tba')
	...
	parser.add_argument('--log-interval', type=int, default=400, help='how many batches to wait before logging the ' \
							'training status')
	...
	# Load data
	train_loader = get_FashionMNIST_dataloader(args.batch_size)

	# Main body
	model = DCN(args)    
	rec_loss_list, acc_list, pur_list, nmi_list, ari_list = solver(args, model, train_loader, train_loader)
